...
Understanding how human genetic variation affects health is a key challenge in precision medicine. Changes in protein-coding regions of DNA, such as missense or loss-of-function variants, can alter protein stability, disrupt biological functions, and interfere with molecular interactions. These variants are often linked to rare diseases and inherited disorders. However, testing the effects of these genetic changes in the lab is both expensive and time-consuming. This has driven the need for computational tools that can help prioritize which variants should be studied further.

This capstone project focuses on a practical, prediction-based machine learning task. It involves building a binary classifier that labels coding variants as either pathogenic or benign. The model will use embeddings from pretrained protein language models as input. The goal is not to uncover the full biological mechanism behind each variant, but to develop a reproducible machine learning workflow that can effectively guide future lab experiments and clinical evaluations.
...
From an ethical and societal standpoint, the project uses only public, de-identified genetic data (Landrum et al., 2018). It makes clear that the model’s predictions are not to be used for clinical diagnosis without further lab testing and expert review. The project also addresses common data challenges, such as class imbalance and uncertain labels, by removing ambiguous cases and using performance metrics that remain meaningful even when classes are uneven.
...
The main dataset for this project is a curated public collection of coding variants with known pathogenicity, assembled by Dr. Fan’s group and shared via Dylan Tan. It includes both clinical labels and precomputed feature representations generated using the ESM2 protein language model. The dataset will be used in accordance with its published terms of use. This dataset is well-suited for a semester-long capstone because it offers a clean foundation for machine learning experimentation while still demanding careful data management, evaluation awareness, and reproducible deployment.

To support reproducibility and provide an independent validation route, the project may also download and process data from ClinVar (https://www.ncbi.nlm.nih.gov/clinvar/). If needed, the dataset can be enriched with consequence annotations from the Ensembl Variant Effect Predictor (VEP) tool (McLaren et al., 2016), which helps confirm how each variant affects coding regions and enables independent construction of training tables similar to the original dataset.
...
Only coding variants will be included. These include mutations that are likely to alter protein-coding transcripts, such as missense, nonsense, or frameshift mutations. When necessary, Ensembl VEP will be used to verify and standardize these variant types across the dataset.

The features used for model training will come from protein language model embeddings. Specifically, the project will use ESM2 embeddings already provided in the curated dataset, which are fixed-length vectors suitable for use with traditional machine learning models. If any variants are missing embeddings, the project will compute them using the ESM2 model based on protein sequence context, and cache them in a consistent format for reproducibility.

Data handling will follow software engineering best practices. The labeled dataset will be saved in versioned Parquet files that record key metadata such as the ClinVar release date (when used), VEP annotation settings, the ESM2 embedding model/version and dimensionality, split seeds/grouping keys, and label filtering choices. All processing steps will be fully scripted and deterministic, ensuring that others can recreate the dataset exactly from the original ClinVar source. Embedding files will be saved in standardized formats to support efficient and reproducible model training without needing to recompute embeddings every time.
...
The final deliverable includes a user-facing application that scores coding genetic variants and returns calibrated pathogenicity probabilities. The application will be implemented as a Streamlit web application and will support two primary workflows: (1) single-variant scoring via a simple input form (e.g., variant identifier fields used to match a record with a precomputed embedding, and/or transcript/protein identifier when available) and (2) batch scoring via CSV upload. For batch input, the app will return a ranked “variant prioritization” table (sorted by predicted pathogenicity probability), with downloadable results suitable for downstream review.
